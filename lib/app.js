...
class Server {
  constructor(_config) {
    ...
    app.get(
      ...
      this.middleware.getCategory.bind(this.middleware),
      (req, res, next) => {
    ...
    app.get("/v1/items/:id([0-9]+)", this.middleware.getItem.bind(this.middleware), (req, res, next) => {
      res.status(200).send(req.item)
      next()
    })

    app.get("/v1/items/:name([a-zA-Z]+)", this.middleware.getItem.bind(this.middleware), (req, res, next) => {
      res.status(200).send(req.item)
      next()
    })
